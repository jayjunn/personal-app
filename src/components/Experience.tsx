    ...
    <PageWrap
      ...
      <ul className="w-full grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 list-none p-0">
        {displayedExperiences.map(({ role, company, description, stacks, id }, index) => (
          <li
            className="flex flex-col justify-start items-start p-5 sm:p-6 border-2 border-black bg-[#e7e2d0] shadow-[3px_3px_0px_#000000] hover:shadow-[5px_5px_0px_#000000] transition-all gap-4"
            key={`exp-${id || company}-${index}`}
            id={company.toLowerCase().replace(/\s+/g, '-')}>
            <div className="w-full flex items-baseline justify-start flex-wrap gap-2 border-b border-black pb-3">
            ...
            <ul className="flex flex-col gap-2.5 text-xs sm:text-sm leading-relaxed font-medium text-black w-full list-none p-0 m-0">
            ...
            {stacks && (
              <div className="flex flex-wrap gap-2 mt-auto pt-4 border-t border-black/10 w-full">
                {stacks.map((stack, sIdx) => (
                  <span
                    key={`exp-stack-${company}-${stack}-${sIdx}`}
                    className="text-[11px] font-bold border border-black rounded px-2.5 py-1 bg-[#f5f0df] text-black">
                    {stack}
                  </span>
                ))}
              </div>
